...
import com.theost.wavenote.utils.SearchSnippetFormatter;
import com.theost.wavenote.utils.SearchTokenizer;
...
public class NoteListFragment extends ListFragment implements AdapterView.OnItemLongClickListener, AbsListView.MultiChoiceModeListener, Bucket.Listener<Preferences> {
    ...
    public static final String ACTION_NEW_NOTE = "com.theost.wavenote.NEW_NOTE";
    /**
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mBucketPreferences = ((Wavenote) requireActivity().getApplication()).getPreferencesBucket();
        mBucketTag = ((Wavenote) requireActivity().getApplication()).getTagsBucket();
    }
    ...
    private ObjectCursor<Note> queryNotesForSearch() {
        ...
        Query<Note> query = Note.all(((Wavenote) requireActivity().getApplication()).getNotesBucket());
        String searchString = mSearchString;
    ...
    public void addNote() {
        ...
        Bucket<Note> notesBucket = wavenote.getNotesBucket();
        ...
        if (DisplayUtils.isLargeScreenLandscape(getActivity())) {
            ...
            requireActivity().startActivityForResult(editNoteIntent, Wavenote.INTENT_EDIT_NOTE);
        }
    }
