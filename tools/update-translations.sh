...
for line in $(grep -v en-rUS $LANG_FILE) ; do
    ...
    curl -sSfL --globoff "https://translate.wordpress.com/projects/wavenote%2Fandroid/$code/default/export-translations?filters[status]=current&format=android" | sed $'s/\.\.\./\…/' | sed $'s/\t/    /g' | sed $'s/\\\@/\@/g' > $RESDIR/values-$local/strings.xml || (echo Error downloading $code && rm -rf $RESDIR/values-$local/)
