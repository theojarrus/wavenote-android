...
function fetch() {
  ...
  mkdir -p $TMPDIR
  curl -sSfL --globoff -o $TMPDIR/strings-$code.xml "https://translate.wordpress.com/projects/wavenote%2Fandroid%2Frelease-notes/$code/default/export-translations?filters[status]=current&format=android" || (echo Error downloading $code)
  done
}
